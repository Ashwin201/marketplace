...
const EditWork = ({ work, workId }) => {
  ...
  }, [session, work.category, work.title, work.description, work.price]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/work/new/${workId}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        ...
        toast.success("Congratulations! Product Updated successfully.");
        router.push(`/product-detail/${workId}`);
      }
    } catch (error) {
    ...
    <>
      ...
      <section className=" flex gap-10 flex-col">
        ...
        <form className=" flex flex-col gap-5" onSubmit={handleSubmit}>
            ...
            >
              {images.length > 0 &&
                images?.map((image, index) => (
                  <div
                    key={index}
                    className=" mr-1 sm:mr-0  col-span-1 rounded-md border-1 border-gray-200 dark:border-gray-800 "
                  >
          ...
          {session?.user?._id !== session?.user?.creator?.work?._id && (
            <button
              type="submit"
        ...
        {session?.user?._id !== session?.user?.creator?.work?._id && (
          <button
            ...
            className=" ml-auto -mt-[74px] flex items-center gap-1 hover:scale-[.99] py-2 px-4 w-fit rounded-md transition-all duration-300 cursor-pointer text-white bg-red-700 hover:bg-red-800 "
          >
            <MdDelete size={20} />{" "}
